...
class SAC():
    def __init__(self, id, alpha=0.0003, beta=0.0003, input_dims=[14],
            ...
            layer1_size=256, layer2_size=256, batch_size=256, reward_scale=15, chkpt_dir='models'):
        self.gamma = gamma
        self.tau = tau
        ...
        self.actor = ActorNetwork(alpha=alpha, input_dims=np.array(input_dims)-1, n_actions=n_actions,
                    name='actor', max_action=1, id=id, chkpt_dir=chkpt_dir)
        self.critic_1 = CriticNetwork(beta=beta, input_dims=input_dims, n_actions=n_actions,
                    name='critic_1', id=id, chkpt_dir=chkpt_dir)
        self.critic_2 = CriticNetwork(beta=beta, input_dims=input_dims, n_actions=n_actions,
                    name='critic_2', id=id, chkpt_dir=chkpt_dir)
        self.value = ValueNetwork(beta=beta, input_dims=input_dims, name='value', id=id, chkpt_dir=chkpt_dir)
        self.target_value = ValueNetwork(beta=beta, input_dims=input_dims, name='target_value', id=id, chkpt_dir=chkpt_dir)
