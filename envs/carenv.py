...
import numpy as np
import time
from envs.rosport.car import RosCarPort
...
class CAREnv(gym.Env):
    ...
    def _check_if_done(self):
        ...
        if self.agents[0].done == True:
            self.game_over = True
        if all([ag.done for ag in self.agents if ag.group == 1]):
            self.game_over = True
        return dones
        ...
        for ag in self.agents:
            if ag.group == 1:
                ag.state = np.array([ag.error[0][0], ag.error[0][1], ag.v_x, ag.w_z, 
                                    self.agents[0].v_x, self.agents[0].w_z, 
    ...
    def get_avg_success_rate(self):
        ...
        得到近100回合平均成功率
        """
        avg_rate = np.mean(self.success_record[-100:])
        return avg_rate
